...
the server */
  ...
  //    "videotestsrc ! video/x-raw-yuv,width=320,height=240,framerate=2/1 ! "
  //    "x264enc ! queue ! rtph264pay name=pay0 pt=96 ! audiotestsrc ! audio/x-raw-int,rate=8000 ! alawenc ! rtppcmapay name=pay1 pt=97 "")");

  //gst_rtsp_media_factory_set_launch (factory,

  //    "( imxv4l2src device=/dev/video2 ! video/x-raw-yuv, framerate=15/1 ! vpuenc codec=6 ! queue ! rtph264pay )");

  gst_rtsp_media_factory_set_launch (factory,
      "( imxv4l2src device=/dev/video2 ! videoscale ! video/x-raw-yuv,width=640,height=480 ! videorate ! video/x-raw-yuv,framerate=15/1 ! rtpvrawpay name=pay0 pt=96 )");
